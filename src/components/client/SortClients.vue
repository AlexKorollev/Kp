    ...
    <div class="profile-edit-block-admin">
      ...
      <div class="profile-edit-main-admin">
        <div class="form-group-admin">
          <label for="search">Поле для сортировки</label>
          <div>
            <select name="search" id="search" v-model="fuild"> 
              <option disabled value="">Выберете поле для сортировки</option>
        ...
        <div class="form-group-admin">
          <label for="addPassword">Сортировать</label>
          <div>
          ...
          <div class="button-group-admin">
            <button class="btn submit-post submit-post-admin" type="submit" @click="sort()" >{{ $t('submitButton') }}</button>
          </div>
        </div>
      </div>
    </div>
    ...
    sort () {
      ...
      if(this.fuild.length==0){
        ...
        const options = {
          ...
          url: 'http://localhost:3000/clients?&_sort=' + this.fuild + '&_order=' + this.sortBy + '&_page='+this.$store.state.currentPage + '&_limit='+ this.$store.state.perPage,
        };
